...
Register the Homebrew-managed plugin directory with Herdr:

```sh
herdr plugin link "$(brew --prefix tabby)/share/tabby"
```

This is the v1 release path. Homebrew installs the `tabby` binary and the release Herdr manifest; Herdr registration remains an explicit user command.
...
Expected output for `v0.1.0` is shaped like:

```text
true
/opt/homebrew/Cellar/tabby/0.1.0/share/tabby
...
- `enabled` is `true`.
- `plugin_root` is under Homebrew's `tabby` Cellar or opt prefix, ending in `share/tabby`.
...
- `herdr plugin link "$(brew --prefix tabby)/share/tabby"` is the separate opt-in registration step.
- Tabby does not silently auto-update. Updates happen through Homebrew, for example `brew upgrade tabby`.
...
Tabby v0.1.0 does not provide a separate `tabby stop` command. To stop a running daemon action, close the running Herdr plugin action/pane if it is visible in Herdr, or terminate the `tabby start` process from a shell:
